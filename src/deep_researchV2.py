from typing import TypedDict, Union,Any,Optional,List , Annotated
...
class DeepState(TypedDict):
    """ overall state of the deepresearch system"""
...
def planner_orchestrator(state: DeepState, config: RunnableConfig):

    llm = ChatOpenAI(model="gpt-5-nano",temperature=0) #for react agent must have "free" llm, no structured output

    planner_orchestrator = create_react_agent(
        llm,
        tools = None,
...
def main():
    ...
    wf = StateGraph(DeepState)
    wf.add_node("planner_orchestrator")
    #wf.add_node("")

    wf.add_edge(START,"planner_orchestrator")
    wf.add_edge("planner_orchestrator",END)
    ...
    with open("artifacts/deepResearchV2.png", "wb") as f:
        f.write(png)
